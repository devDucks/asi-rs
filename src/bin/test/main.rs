...
fn get_roi(idx: i32) {
    ...
    let mut bin = 20;
    let mut img_type = 20;
    let _ = libasi::camera::get_roi_format(idx, &mut width, &mut height, &mut bin, &mut img_type);
    println!(
        "Width: {}\nHeight: {}\nBin: {}\nType: {}",
...
fn expose(idx: i32) -> u32 {
    let mut e_val: i64 = 0;
    let _ = libasi::camera::get_control_value(
        idx,
    ...
    println!("Exposing");
    let _ = libasi::camera::start_exposure(idx);

    let mut status = 0;
    let _ = libasi::camera::exposure_status(idx, &mut status);
...
fn main() {
    ...
    for idx in 0..num_of_devs {
        ...
        let _ = libasi::camera::open_camera(idx);
        let _ = libasi::camera::init_camera(idx);

        let mut num_of_controls = 0;
        let _ = libasi::camera::get_num_of_controls(idx, &mut num_of_controls);
        ...
        for c_id in 0..num_of_controls {
            ...
            let _ = libasi::camera::get_control_caps(idx, c_id, &mut control_caps);
        ...
        let _ = libasi::camera::get_start_position(idx, &mut start_x, &mut start_y);
        ...
        let _ = libasi::camera::get_camera_mode(idx, &mut cmode);
        println!("Camera mode: {}", cmode);

        let _ = libasi::camera::set_control_value(
            idx,
            libasi::camera::ASI_CONTROL_TYPE_ASI_EXPOSURE as i32,
            length,
        ...
        let _ = libasi::camera::close_camera(idx);
    }
}
